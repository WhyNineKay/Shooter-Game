...
class Player:
    ...
    def update(self) -> None:
        ...
        if self.x + self.vx > constants.SCREEN_WIDTH - constants.PLAYER_SIZE:
        ...
        if self.y + self.vy > constants.SCREEN_HEIGHT - constants.PLAYER_SIZE:
        ...
        for wall in globs.walls:
            ...
            if self.x <= wall.x:
                if wall.rect.collidepoint(self.x + self.vx * globs.dt + constants.PLAYER_SIZE, self.y):
                    if self.vx * globs.dt > 0:
                        self.vx = 0

                    if wall.rect.colliderect(self.rect):
                        self.vx = -constants.PLAYER_WALL_ANTICLIP_VELOCITY * globs.dt

            else:
                if wall.rect.collidepoint(self.x + self.vx * globs.dt, self.y):
                    self.vx = 0

            if self.y <= wall.y:
                if wall.rect.collidepoint(self.x, self.y + self.vy * globs.dt + constants.PLAYER_SIZE):
                    if self.vy * globs.dt > 0:
                        self.vy = 0

                    if wall.rect.colliderect(self.rect):
                        self.vy = -constants.PLAYER_WALL_ANTICLIP_VELOCITY * globs.dt

            else:
                if wall.rect.collidepoint(self.x, self.y + self.vy * globs.dt):
                    self.vy = 0
